...
function YouTubePlayer ( arg ) {
		...
		*/
		source : {
			...
			set : function ( value ){
				...
				trace ( 'YouTubePlayer.source:', value)
	...
	*/
	...
	function createPlayer() {
		trace ( '++++++++++ YouTubePlayer.createPlayer()')
		if ( V.player ) {
			V.player.destroy();
		}
	...
	function checkCuePoint() {
		...
		//	trace ( _cuePoints.active, ' checkCuePoint()')
	...
	function handlePlayerReady ( event ){
		...
		trace ( '\t\t after ready fire, muted:', V.muted )
		if( arg.autoPlay ) V.play();
	}

	function handleStateChange ( event ){
		trace ( ' -- YoutubePlayer stageChange()' );
		switch( event.data ) {
			...
			case 3:
				trace ( '\t BUFFERING = 3' )
				fireEvent( MediaEvent.WAITING, V.onBuffer, event );
				break;
	...
	function handleFail( event ) {
		trace ( 'YouTubePlayer.handleFail()' )
	...
	trace ( 'after create, muted:', V.muted )
