/**
	...
	@desc
		...
		<codeblock>
			...
			// access button 
			adData.elements.videoPlayer.controls.buttonPlayPause;
		</codeblock>
		<br><br>
		...
		<codeblock>
			...
			// access button 
			adData.elements.videoPlayer.controls.controlBar.buttonPlayPause;
		</codeblock>
		<br><br>
...
import { GestureEvent, UIEvent } from 'ad-events'
...
function VideoControls ( player, arg ){
	...
	Styles.injectStylesheet ( 'RED_uiVideoControls', 
		'.rvp-controls', 'width:inherit; height:inherit; position:absolute; display:table;',
		'.rvp-screen-btn', 'width:100%;	height:100%;',
		'.rvp-controlBar-container', 'position:relative; width:inherit; height:inherit; vertical-align:bottom; display:table-cell;',
		...
		'.rvp-controlBar-container .ui-button', 'width:30px; height:30px;',
		'.rvp-controlBar-container .ui-button-state', 'background-size: 75%; background-position:50% 50%;',
		...
		'.rvp-progress-non-inline', 'position:relative;',
		'.rvp-progress-non-inline .ui-slider-bg, .rvp-progress-non-inline .ui-slider-loaded, .rvp-progress-non-inline .ui-slider-handle, .rvp-progress-non-inline .ui-slider-track,  .rvp-progress-non-inline .ui-slider-hitState', 'bottom:0px;'
	)
		...
		for ( var i = 0; i < _includeList.length; i++ ){
			...
			if ( scope[name] != undefined && scope[name] != false ) {
				...
				target [ name ] = function(){
					switch ( name ){
						case 'buttonPlayPause' 	: 	return new UIButtonPlayPause ( player, params );
						case 'buttonReplay' 	: 	return new UIButtonReplay ( player, params );
						case 'buttonMute'		: 	return new UIButtonMute ( player, params );
						case 'buttonFullScreen'	: 	return new UIButtonFullScreen ( player, params );
						case 'sliderVolume'		: 	return new UISliderVolume ( player, params );
						case 'sliderProgress'	: 	return new UISliderProgress ( player, params );
						case 'timeDisplay'		: 	return new UITimeDisplay ( player, params );
					}					
				}()
	...
	Object.defineProperties ( U, {
		/**
			...
			@example
				// GET
				trace ( myVideoPlayer.controls.replayOnInteraction );
		...
		*/
		...
		/**
			...
			@example
				// GET
				trace ( myVideoPlayer.controls.unmuteOnInteraction );
	...
	function animateIn(){
		...
		if ( U.controlBar )
			TweenLite.to ( _controlBarTargets, .3, { alpha:1 })
	}
	...
	function handleMove ( event ){
		...
		if ( !player.complete && player.hasPlayed )
			animateIn();
	}
	...
	Gesture.addEventListener ( player.container, GestureEvent.CLICK, handleClickBubbleStop );
