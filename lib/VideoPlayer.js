...
function VideoPlayer(arg) {
	...
	Object.defineProperties(V, {
		...
		*/
		source: {
			get: () => video.src,
			set: value => {
				console.log('set video source to:', value)
				...
				// if using the mpg, don't set the source. it throws errors
				// if ((_hasMpg && _deviceBrand == 'android') || _forceMpg) return
				if (_forceMpg) return

				console.log('bypassed and setting the video.src')
				video.src = value[0]
			}
		},
	...
	*/
	V.play = () => {
		console.log('VideoPlayer.play(), video.paused:', video.paused, 'video.muted:', video.muted)
		addIESource()
