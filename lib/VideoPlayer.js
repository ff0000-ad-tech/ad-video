...
function VideoPlayer(arg) {
		...
		autoPlay: {
			...
			set: value => {
				...
				if (value) {
					...
					if (_deviceType != 'desktop' && _deviceBrand == 'android') {
						// we also want to load the mpg player even if we aren't autoPlaying it
						// load MpegPlugin player, or assign player
						loadMpgPlayer()
					} else {
	...
	*/
	V.addCuePoint = (time, handler, params) => {
		...
		// console.log('addCuePoint:', _cuePoints)
	}
	...
	*/
	...
	function checkCuePoint() {
		// console.log('checkCutPoint', _cuePoints.active, _cuePoints.pool.length)
	...
	function loadMpgPlayer() {
		console.log('VideoPlayer.loadMpgPlayer(), window.MpegPlugin:', window.MpegPlugin, ', _hasMpg:', _hasMpg)
		if (window.MpegPlugin == undefined && _hasMpg) {
			new InlineLoader(runtimeIncludes.mpegPluginPath, {
				onComplete: function(event) {
					console.log('MpegPlugin load complete')
					V.screen = video = _mpeg = new MpegPlugin({
						...
						autoplay: false,
						onReady: function() {
							console.log('MpegPlugin.onReady()')
							handleCanPlay()
							V.hasPlayed = _autoPlay
							if (_autoPlay) V.play()
						},
						onComplete: function() {
							//console.log( '_mpeg.onComplete()');
	...
	function handleEventKill(event) {
		/*if ( event.type == 'seeking' || event.type == 'seeked' ){ 
			console.log( ':: Event Kill :: seeked|seeking' )
		}else{ 
			console.log( ':: Event Kill ::', event.type )
		}*/
